...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]){
        ...
        _leftImage.frame = CGRectMake(5, 5, self.height-10, self.height-10);
        _leftImage.contentMode = UIViewContentModeScaleAspectFit;
        [backV addSubview:_leftImage];
        
        _leftLabel = [[UILabel alloc]init];
        _leftLabel.frame = CGRectMake(_leftImage.rightX+10, 0, backV.width/2-10, backV.height);
        _leftLabel.textAlignment = NSTextAlignmentLeft;
        _leftLabel.font = Label_font(15.f);
        _leftLabel.textColor = HexRGB(0xffffff);
        [backV addSubview:_leftLabel];
        
        _rightLablel = [[UILabel alloc]init];
        _rightLablel.frame = CGRectMake(backV.width/2, 0, backV.width/2-5, backV.height);
        _rightLablel.font = Label_font(15.f);
        _rightLablel.textAlignment = NSTextAlignmentRight;
        _rightLablel.textColor = HexRGB(0xffffff);
...
-(void)loadFromDta:(NSArray *)array{
    ...
    [detailTV reloadData];
    labelR.text = [NSString stringWithFormat:@"%@",[array valueForKeyPath:@"@sum.spendcount"] ];
}
-(void)loadSubViewS{
...
-(void)layoutSubviews{
    detailTV.frame = CGRectMake(0,labelL.bottomY,self.width,self.height-labelL.bottomY);
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    XPIModel *model = [_dataArray objectAtIndex:indexPath.row];
    NSString *imageStr =[NSString stringWithFormat:@"%@-w",[Help getImageName:model]];
    cell.leftImage.image = IMAGE_NAMED(imageStr);
    cell.leftLabel.text = [Help getTypeChineseName:model];
    cell.rightLablel.text = model.spendcount;
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    [_delegate showEditViewControllerWithDataModel:[_dataArray objectAtIndex:indexPath.row]];
}
/*
